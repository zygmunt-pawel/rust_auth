...
use common::{CapturingMailer, CapturingSink, loopback_ip, test_builder, test_config};

async fn issue_and_get_code(
    pool: &sqlx::PgPool,
    mailer: &common::CapturingMailer,
    email: &str,
) -> String {
    let cfg = test_config();
    auth_rust::store::issue_magic_link(pool, email, loopback_ip(), &cfg, mailer)
        .await
        .unwrap();
...
async fn verify_with_wrong_code_increments_attempts(pool: sqlx::PgPool) {
    ...
    let attempts: i32 =
        sqlx::query_scalar("SELECT code_attempts FROM magic_links WHERE email = $1")
            .bind("u@e.com")
            .fetch_one(&pool)
            .await
            .unwrap();
