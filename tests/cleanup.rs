...
async fn cleanup_removes_old_magic_links_keeps_fresh(pool: sqlx::PgPool) {
    ...
    for n in 0..3 {
        auth_rust::store::issue_magic_link(
            &pool, &format!("u{n}@e.com"), loopback_ip(), &cfg, &*mailer,
        ).await.unwrap();
    }
    ...
    let report = auth_rust::store::cleanup_expired(&pool).await.unwrap();
    assert_eq!(report.magic_links_deleted, 5, "5 old rows should be deleted");
...
async fn cleanup_removes_dead_sessions_keeps_live(pool: sqlx::PgPool) {
    ...
    auth_rust::store::verify_magic_link_or_code(
        &pool,
        VerifyInput::Code { email: Email::try_from("u@e.com".to_string()).unwrap(), code: VerifyCode::from_string(code) },
    ...
    let dead_user_id: i64 = sqlx::query_scalar(
        "INSERT INTO users (email) VALUES ('dead@e.com') RETURNING id"
    ).fetch_one(&pool).await.unwrap();
    sqlx::query(
    ...
    ).bind(dead_user_id).execute(&pool).await.unwrap();

    let before: i64 = sqlx::query_scalar("SELECT COUNT(*) FROM sessions").fetch_one(&pool).await.unwrap();
    assert_eq!(before, 2);

    let report = auth_rust::store::cleanup_expired(&pool).await.unwrap();
    assert_eq!(report.sessions_deleted, 1);

    let after: i64 = sqlx::query_scalar("SELECT COUNT(*) FROM sessions").fetch_one(&pool).await.unwrap();
